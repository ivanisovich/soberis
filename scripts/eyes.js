document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('mousemove', (event) => {
    const svg = document.querySelector('.eyes');
      ...
      const ratioX =  svg.getBoundingClientRect().width/1527;
      const ratioY =  svg.getBoundingClientRect().height/706;

      const { clientX, clientY } = event;
      const maxMove = window.innerWidth * 0.01; // 1vw in pixels
      eyes.forEach(eye => {
          const dx = clientX/ratioY - eye.cx;
          const dy = clientY/ratioX - eye.cy;
